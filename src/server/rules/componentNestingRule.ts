...
export const componentNestingRule: Rule = {
  ...
  description: "Ensures component nesting constraints are respected (e.g., no nested <Script> tags, no <WidgetPlaceholder> inside <Script> tags).",
      ...
      while (parent) {
        ...
        if (parentTagName === "Script") {
          if (tagName === "Script") {
            diagnostics.push({
              code: "streak:S602",
              message: "Nesting `<Script>` tags inside other `<Script>` tags is not allowed.",
          ...
          if (tagName === "WidgetPlaceholder") {
            diagnostics.push({
              code: "streak:S602",
              message: "Nesting `<WidgetPlaceholder>` inside a `<Script>` tag callback is not allowed as widget injection is a server-side framework function.",
              range: getRangeFromNode(sourceFile, node),
              severity,
              source: "Streak Engine",
            });
